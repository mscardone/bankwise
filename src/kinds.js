...
(function (root, factory) {
  ...
  var RULES = [
    ...
    ["prayer", /bones?$|bones? \(|\bashes$|\bbonemeal\b|ensouled|\bincense\b|impious|accursed|infernal ashes|searing ashes|tortured ashes/i, /^bones$|^ashes$|prayer items|remains/i],
    ["wood", /\blogs?$|\bplanks?$|\bshafts?$|\bshieldbow \(u\)|\bshortbow \(u\)|\(u\)$|bow ?string|\bflax\b|\bbranches\b|\broots?$|timber|refined plank|headless arrow|feathers?$|elder wood|\bstock$/i, /^logs$|^planks$|fletching|woodcutting|firemaking|construction materials/i],
  ...
  function kindOf(name, cats) {
    var i, j, r, n = String(name || "");
    cats = cats || [];
    for (i = 0; i < RULES.length; i++) {
      r = RULES[i];
      if (r[1] && r[1].test(n)) return r[0];
    }
    for (i = 0; i < RULES.length; i++) {
      r = RULES[i];
  ...
  function tabFor(kind, templateId) {
